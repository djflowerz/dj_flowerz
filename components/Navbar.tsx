        ...
        <AnimatePresence>
          {isOpen && (
            <motion.div 
              ...
              className="fixed inset-0 top-0 bg-[#050505] z-[110] lg:hidden flex flex-col"
            >
              <div className="p-4 flex justify-end">
                <button onClick={() => setIsOpen(false)} className="p-4 text-white/40 hover:text-white">
                    <X size={32} />
                </button>
              </div>
              <div className="flex-1 overflow-y-auto px-6 py-4 space-y-3">
                {navLinks.map((link) => {
                  ...
                  return (
                    ...
                    >
                      <div className={`w-8 h-8 rounded-lg flex items-center justify-center ${isActive ? 'bg-[#A349F5]/20' : 'bg-black'}`}>
                ...
                <div className="pt-4 space-y-3">
                  <div className="text-[10px] font-black text-white/20 uppercase tracking-[0.3em] px-2 mb-2">Discovery</div>
                  <Link to="/search" className="flex items-center gap-4 p-4 rounded-2xl bg-white/5 border border-white/5 text-white/60">
                  ...
                  {isAuthenticated && (
                    <>
                      <Link to="/messages" className="flex items-center gap-4 p-4 rounded-2xl bg-white/5 border border-white/5 text-white/60">
                        <div className="w-8 h-8 rounded-lg bg-black flex items-center justify-center">
                          <Mail size={20} className="text-[#A349F5]" />
                        </div>
                        <span className="text-lg font-bold">Messages</span>
                      </Link>
                      <Link to="/notifications" className="flex items-center gap-4 p-4 rounded-2xl bg-white/5 border border-white/5 text-white/60">
                        <div className="w-8 h-8 rounded-lg bg-black flex items-center justify-center relative">
                ...
                <div className="pt-6 grid grid-cols-2 gap-3">
                  ...
                  {isAuthenticated ? (
                    <>
                      <Link to="/account" className="col-span-2">
                        <button className="w-full bg-white/5 text-white font-bold py-4 rounded-2xl border border-white/10 uppercase tracking-widest text-sm">
                          ACCOUNT SETTINGS
                        </button>
                      </Link>
                      <button 
                        onClick={logout}
                        className="col-span-2 w-full bg-red-500/10 text-red-500 font-bold py-4 rounded-2xl border border-red-500/20 uppercase tracking-widest text-sm"
                      >
                        LOGOUT
                      </button>
                    </>
