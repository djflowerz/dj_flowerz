...
export const templates = {
  ...
  // 16. Escrow Release — Admin Payout Alert
  escrowRelease: (escrowId, itemDescription, buyerEmail, sellerEmail, amountKes, feeKes, sellerReceives) => emailWrap(`
    ${heroSection('💰', 'Escrow Release Required', 'A buyer has confirmed delivery. Please process the seller payment.')}
    ${divider}
    <div style="background:linear-gradient(135deg,#a855f720,#4ade8010);border:1px solid #a855f730;border-radius:16px;padding:28px 32px;margin:24px 0;text-align:center;">
      <p style="margin:0 0 4px;font-size:11px;font-weight:900;letter-spacing:4px;text-transform:uppercase;color:#6b7280;">Your Commission</p>
      <p style="margin:0;font-size:44px;font-weight:900;color:#a855f7;letter-spacing:-1px;">KES ${feeKes.toLocaleString()}</p>
      <p style="margin:8px 0 0;font-size:12px;color:#6b7280;">Deducted from total before seller payment</p>
    </div>
    ${infoCard([
      ['Escrow ID', escrowId],
      ...
      ['Buyer', buyerEmail],
      ['Seller', sellerEmail],
      ['Total Paid by Buyer', `KES ${amountKes.toLocaleString()}`],
      ['Platform Fee (Your Cut)', `<span style="color:#a855f7;font-weight:900;">KES ${feeKes.toLocaleString()}</span>`],
      ['Amount to Pay Seller', `<span style="color:#4ade80;font-weight:900;">KES ${sellerReceives.toLocaleString()}</span>`],
    ], '#4ade80')}
    <div style="background:#0f1a0a;border:1px solid #4ade8025;border-radius:16px;padding:20px 32px;margin:24px 0;">
      <p style="margin:0 0 8px;font-size:13px;font-weight:700;text-transform:uppercase;letter-spacing:2px;color:#4ade80;">Action Required</p>
      <p style="margin:0;font-size:14px;color:#9ca3af;line-height:1.7;">Process a M-Pesa / Bank transfer of <strong style="color:#ffffff;">KES ${sellerReceives.toLocaleString()}</strong> to the seller's registered payout account. Once done, mark as paid in the admin panel.</p>
    </div>
    ${ctaButton('https://djflowerz.co.ke/admin', '🔓 Open Admin Panel → Process Payout', 'cyan')}
    <p style="text-align:center;margin:16px 0 0;font-size:12px;color:#374151;">Auto-generated by DJ Flowerz Escrow Engine · ${new Date().toLocaleString('en-KE', { timeZone: 'Africa/Nairobi' })}</p>
  `),

  // 17. Escrow Release — Seller Notification
  escrowReleaseSeller: (sellerName, itemDescription, sellerReceives, feeKes) => emailWrap(`
    ${heroSection('🎉', 'Your Payment is Being Released!', `Great news <strong style="color:#ffffff;">${sellerName}</strong>! The buyer has confirmed delivery.`)}
    ${divider}
    <div style="background:linear-gradient(135deg,#4ade8015,#a855f708);border:1px solid #4ade8025;border-radius:16px;padding:28px 32px;margin:24px 0;text-align:center;">
      <p style="margin:0 0 4px;font-size:11px;font-weight:900;letter-spacing:4px;text-transform:uppercase;color:#6b7280;">Amount Coming Your Way</p>
      <p style="margin:0;font-size:44px;font-weight:900;color:#4ade80;letter-spacing:-1px;">KES ${sellerReceives.toLocaleString()}</p>
      <p style="margin:8px 0 0;font-size:12px;color:#6b7280;">After KES ${feeKes.toLocaleString()} platform fee</p>
    </div>
    ${infoCard([
      ['Item Sold', itemDescription],
      ['Platform Fee', `KES ${feeKes.toLocaleString()}`],
      ['Your Payout', `<span style="color:#4ade80;font-weight:900;">KES ${sellerReceives.toLocaleString()}</span>`],
      ['Status', '<span style="color:#facc15;">⏳ Admin Processing</span>'],
    ])}
    <div style="background:#15151f;border:1px solid #ffffff08;border-radius:16px;padding:20px 32px;margin:24px 0;">
      <p style="margin:0;font-size:14px;color:#9ca3af;line-height:1.7;">Our admin is processing your M-Pesa / Bank transfer now. You should receive your funds within <strong style="color:#ffffff;">24 hours</strong>. If you have any questions, reply to this email.</p>
    </div>
    ${ctaButton('https://djflowerz.co.ke/escrow-mngt', '📊 View Escrow Dashboard', 'primary')}
    <p style="text-align:center;margin:16px 0 0;font-size:13px;color:#6b7280;">Thanks for selling on DJ Flowerz 🇰🇪</p>
  `),
