...
const Cart: React.FC = () => {
   ...
   const hasPhysicalItems = useMemo(() => items.some(item => {
      ...
      // Explicitly check for digital type or if shipping is not required
      if (item.type === 'digital' || item.requiresShipping === false || ['digital', 'software', 'samples'].includes(cat)) return false;
      // Default to physical
      return true;
   }), [items]);
   ...
   const paystackConfig = {
      ...
      email: email,
      amount: finalTotal * 100, // Paystack expects amount in smallest currency unit (cents for KES/NGN)
      ...
      metadata: {
         ...
         shippingAddress: deliveryType === 'door'
            ? `${address}, ${landmark ? landmark + ', ' : ''}${city} ${postalCode}`
         ...
         custom_fields: [
            ...
            {
               display_name: "Shipping Address", variable_name: "shipping_address", value: deliveryType === 'door'
                  ? `${address}, ${city}`
                  : `Pickup: ${pickupStation}`
            }
         ]
   ...
   const handleCheckout = (e: React.FormEvent) => {
      ...
      if (hasPhysicalItems) {
         if (deliveryType === 'door' && (!city || !address)) {
            alert("Please provide your delivery city and street address.");
            return;
         }
         if (deliveryType === 'pickup' && !pickupStation) {
            alert("Please specify a pickup station.");
            return;
         }
      }
      ...
      // If order is free, bypass payment
      if (finalTotal === 0) {
         onSuccess({ reference: `FREE-${Date.now()}` });
      } else {
               ...
               <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                     ...
                     {hasPhysicalItems ? (
                           ...
                           <form id="checkout-form" onSubmit={handleCheckout} className="space-y-4 pt-6 border-t border-white/5">
                              ...
                              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                 <div>
                                    ...
                                    <input required type="text" value={firstName} onChange={e => setFirstName(e.target.value)} className="w-full bg-black/20 border border-white/10 rounded-lg p-3 text-white focus:border-brand-purple focus:outline-none" />
                                 </div>
                                 <div>
                                    <label className="block text-xs text-gray-500 mb-1 font-bold">Last Name</label>
                                    <input required type="text" value={lastName} onChange={e => setLastName(e.target.value)} className="w-full bg-black/20 border border-white/10 rounded-lg p-3 text-white focus:border-brand-purple focus:outline-none" />
                                 </div>
                              </div>
                              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                 <div>
                                    <label className="block text-xs text-gray-500 mb-1 font-bold">Email Address</label>
                                    <input required type="email" value={email} onChange={e => setEmail(e.target.value)} className="w-full bg-black/20 border border-white/10 rounded-lg p-3 text-white focus:border-brand-purple focus:outline-none" />
                                 </div>
                                 <div>
                                    <label className="block text-xs text-gray-500 mb-1 font-bold">Phone Number</label>
                                    <input required type="tel" value={phone} onChange={e => setPhone(e.target.value)} className="w-full bg-black/20 border border-white/10 rounded-lg p-3 text-white focus:border-brand-purple focus:outline-none" />
                                 </div>
                              </div>
                                 ...
                                 <>
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                       <div>
                                          ...
                                          <input required type="text" value={city} onChange={e => setCity(e.target.value)} className="w-full bg-black/20 border border-white/10 rounded-lg p-3 text-white focus:border-brand-purple focus:outline-none" />
                                       </div>
                                       <div>
                                          <label className="block text-xs text-gray-500 mb-1 font-bold">Postal Code</label>
                                          <input type="text" value={postalCode} onChange={e => setPostalCode(e.target.value)} className="w-full bg-black/20 border border-white/10 rounded-lg p-3 text-white focus:border-brand-purple focus:outline-none" />
                                       </div>
                                    </div>
                                    <div>
                                       <label className="block text-xs text-gray-500 mb-1 font-bold">Street Address / Location</label>
                                       <input required type="text" value={address} onChange={e => setAddress(e.target.value)} placeholder="Street, Building, House No." className="w-full bg-black/20 border border-white/10 rounded-lg p-3 text-white focus:border-brand-purple focus:outline-none" />
                                    </div>
                                    <div>
                                       <label className="block text-xs text-gray-500 mb-1 font-bold">Landmark / Delivery Instructions</label>
                                       <input type="text" value={landmark} onChange={e => setLandmark(e.target.value)} placeholder="e.g. Next to Total Gas Station" className="w-full bg-black/20 border border-white/10 rounded-lg p-3 text-white focus:border-brand-purple focus:outline-none" />
                                    </div>
                                 </>
                              ) : (
                                 <div>
                                    <label className="block text-xs text-gray-500 mb-1 font-bold">Preferred Pickup Station</label>
                                    <input required type="text" value={pickupStation} onChange={e => setPickupStation(e.target.value)} placeholder="Enter preferred Post Office Branch or Agent" className="w-full bg-black/20 border border-white/10 rounded-lg p-3 text-white focus:border-brand-purple focus:outline-none" />
                                 </div>
                              )}
                           </form>
                        </div>
                        ...
                        <div className="bg-[#15151A] p-6 rounded-2xl border border-white/5 flex flex-col md:flex-row items-center gap-4 text-center md:text-left">
                           ...
                           <div>
                              ...
                              <form id="checkout-form" onSubmit={handleCheckout} className="mt-4 space-y-3">
                                 <div>
                                    <label className="block text-xs text-gray-500 mb-1 font-bold">First Name</label>
                                    <input required type="text" value={firstName} onChange={e => setFirstName(e.target.value)} className="w-full bg-black/20 border border-white/10 rounded-lg p-3 text-white focus:border-brand-purple focus:outline-none" />
                                 </div>
                                 <div>
                                    ...
                                    <input required type="email" value={email} onChange={e => setEmail(e.target.value)} placeholder="you@example.com" className="w-full bg-black/20 border border-white/10 rounded-lg p-3 text-white focus:border-brand-purple focus:outline-none" />
                                 </div>
                                 <div>
                                    <label className="block text-xs text-gray-500 mb-1 font-bold">Phone (M-Pesa)</label>
                                    <input required type="tel" value={phone} onChange={e => setPhone(e.target.value)} className="w-full bg-black/20 border border-white/10 rounded-lg p-3 text-white focus:border-brand-purple focus:outline-none" />
                                 </div>
                              </form>
                           </div>
                  ...
                  <div className="lg:col-span-1">
                     <div className="bg-[#15151A] p-6 rounded-2xl border border-white/5 sticky top-24 shadow-xl">
                              ...
                              {appliedCoupon && (
                                 <div className="flex justify-between text-green-400 font-medium animate-fade-in">
                                    <div className="flex items-center gap-2">
                                       ...
                                       <button onClick={removeCoupon} className="text-gray-500 hover:text-red-400"><Trash2 size={12} /></button>
                                    </div>
                        ...
                        {!appliedCoupon && (
                           <div className="mb-6">
                              <label className="block text-xs text-gray-500 mb-2 font-bold uppercase">Have a Coupon?</label>
                              <div className="flex gap-2">
                                 <input
                                    ...
                                    className="flex-1 bg-black/20 border border-white/10 rounded-lg px-3 py-2 text-white text-sm focus:border-brand-purple focus:outline-none"
                                 />
                                 <button
                                    onClick={handleApplyCoupon}
                                    disabled={isValidatingCoupon || !couponCode}
                                    className="px-4 py-2 bg-white/5 border border-white/10 rounded-lg text-white text-sm font-bold hover:bg-white/10 transition disabled:opacity-50"
                                 >
